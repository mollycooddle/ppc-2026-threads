...
FatehovKGaussianOMP::FatehovKGaussianOMP(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool FatehovKGaussianOMP::ValidationImpl() {
  const auto &input = GetInput();
  return input.image.width > 0 && input.image.height > 0 && input.image.channels > 0 && !input.image.data.empty() &&
         input.sigma > 0.0F;
}

bool FatehovKGaussianOMP::PreProcessingImpl() {
  const auto &input = GetInput();
...
bool FatehovKGaussianOMP::RunImpl() {
  const auto &input = GetInput();
  auto &output = GetOutput();
...
bool FatehovKGaussianOMP::PostProcessingImpl() {
  return true;
}
